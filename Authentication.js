...
let transport = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "nassimkhlifisimplonenda@gmail.com", // generated ethereal user
    pass: "jnjwgprzqvxcbaud", // generated ethereal password
  },
});
const runMail = async () => {
  const activeMail = transport.verify((error, success) => {
    if (error) {
      console.log(error);
    } else {
      console.log(success, "Running mail Successfully !!");
    }
  });
  const result = new Promise((resolve, reject) => {
    return resolve(activeMail);
  })
    .then((response) => {
      return response;
    })
...
const emailVerification = async(verifLink,email)=>{
  const verificationLink = `http://localhost:5000/app/verify-email?token=${verifLink}`;
  return await transport.sendMail({
      from: "nassimkhlifisimplonenda@gmail.com",
      to: email,
      ...
      <div style="background-color: #FFFFFF; margin: 0; padding: 0; -webkit-text-size-adjust: none; text-size-adjust: none;">
      ...
      <div align="center" class="alignment" style="line-height:10px"><img src="http://localhost:5000/image" style="display: block; height: auto; border: 0; width: 175px; max-width: 100%;" width="175"/></div>
      </td>
      </tr>
      </table>
      ...
      <h1 style="margin: 0; color: #393d47; direction: ltr; font-family: Tahoma, Verdana, Segoe, sans-serif; font-size: 25px; font-weight: normal; letter-spacing: normal; line-height: 120%; text-align: center; margin-top: 0; margin-bottom: 0;"><strong>Email Verification</strong></h1>
      </td>
      </tr>
      </table>
      ...
      <p style="margin: 0; text-align: center; mso-line-height-alt: 18px;"><strong>Verify your Email To take permission to Log In into your Account</strong></p>
      </div>
      </div>
      </td>
      </tr>
      ...
      <!--[if mso]><v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="www.yourwebsite.com" style="height:58px;width:192px;v-text-anchor:middle;" arcsize="35%" strokeweight="0.75pt" strokecolor="#FFC727" fillcolor="#ff5757"><w:anchorlock/><v:textbox inset="0px,0px,0px,0px"><center style="color:#393d47; font-family:Tahoma, Verdana, sans-serif; font-size:18px"><![endif]--><a href="${verificationLink}" style="text-decoration:none;display:inline-block;color:#393d47;background-color:#ff5757;border-radius:20px;width:auto;border-top:1px solid #FFC727;font-weight:undefined;border-right:1px solid #FFC727;border-bottom:1px solid #FFC727;border-left:1px solid #FFC727;padding-top:10px;padding-bottom:10px;font-family:Tahoma, Verdana, Segoe, sans-serif;font-size:18px;text-align:center;mso-border-alt:none;word-break:keep-all;" target="_blank"><span style="padding-left:50px;padding-right:50px;font-size:18px;display:inline-block;letter-spacing:normal;"><span dir="ltr" style="word-break:break-word;"><span data-mce-style="" dir="ltr" style="line-height: 36px;"><strong>Click Here</strong></span></span></span></a>
...
router.post("/register", async (req, res) => {
  let registerFunction = USER.findOne({ email: req.body.email }).then(
    async (user) => {
      if (user) {
       const verificationToken1 = jwt.sign({ email:user.email  , expiresAt:  Date.now() +2160000 }, "NOONECANGESS");
      return await  emailVerification(verificationToken1,user.email).then(()=>{
          return res.status(400).json({
            error: "User already registered We Have Send You a Verification Email",
          });
        })
      }
      else  {
        ...
        //User Object
        const verificationToken2 = jwt.sign({ email:email , expiresAt:  Date.now() +2160000 }, "NOONECANGESS");
        ...
        return await _user.save().then(async () => {
      return await emailVerification(verificationToken2,email).then(()=>{
        return res.status(200).json({
          message:'Registered successfully , we have Send you a email verification token'
        })
      }).catch((err) => {
        return res.status(404).json({
          message:'Failed to Send  an email verification token',
          error:err
        })
      })
        });
...
router.get("/verify-email", async (req, res) => {
  ...
  try {
    ...
    const { email , expiresAt} = jwt.verify(token, "NOONECANGESS");
    if(expiresAt < Date.now()){
      res.send('This token has expired you should Register again and we will Send You another One ')
    }
    //Verify User
    const verifyedUser = await USER.findOneAndUpdate(
      { email: email },
      {
        $set: {
          isVerified: true,
      ...
      .then(() => {
        return res.send("Your email has been verified. You can now log in.");
      })
      .catch((err) => {
        return res.send(`Problem When trying to Verify Your Email : ${err}`);
      });
  ...
  USER.findOne({ email: req.body.email }).then(async (user) => {
      if (user) {
        if(!user.isVerified){
          ...
          return await  emailVerification(verificationToken1,user.email).then(()=>{
            res.status(401).json({
              status:'Your account is not Verified Check your Email  We Have Send You An Verification Email'
            })
          })
        }else{
          const verPass =  await bcrypt.compare(req.body.password,user.password)
          if (verPass) {
           const token =  jwt.sign(user, "NOONECANGESS")
             res.status(200).json({
                 data:token ,
                 user: user
           });
        ...
        return res.status(400).json({ message: "Something went wrong When Trying to Sign In" });
      }
    });
...
router.post('/reset', async()=>{
  const result = await USER.findOne({ email:req.body.email}).then(async(user)=>{
      if(!user){
        return res.status(404).json({
